...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            wandb.log({"Train Loss": loss.item()})  # 记录训练损失到wandb
...
def test(args, model, device, test_loader):
    ...
    wandb.log({"Test Loss": test_loss, "Test Accuracy": 100. * correct / len(test_loader.dataset)})  # 记录测试损失和准确率到wandb
...
def main():
    ...
    wandb_entity = '2020152035-shenzhenu'
    wandb_project = 'mnist'
    project_name = 'rnn'
    project_name += f"+batch-r{args.batch_size}+learning_rate-r{args.lr}"
    wandb.init(entity=wandb_entity, project=wandb_project, name=project_name)
