...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            swanlab.log({"Train Loss": loss.item()})  # 记录训练损失到wandb
...
def test(args, model, device, test_loader):
    ...
    swanlab.log({"Test Loss": test_loss, "Test Accuracy": 100. * correct / len(test_loader.dataset)})  # 记录测试损失和准确率到swanlab
...
def main():
    ...
    experiment_name = "mnist_rnn"
    experiment_name += f"+batch-r{args.batch_size}+learning_rate-r{args.lr}"
    swanlab.init(project="minst_rnn",config={'learning-rate': args.lr},experiment_name=experiment_name)
