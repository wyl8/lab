...
if __name__ == "__main__":
    ...
    he_data = load_json_data("he_data.json")
    for each_example in he_data:
        task_id = each_example['task_id']
        code_to_test = each_example['code']
        ...
        # 头一次生产单测
        # 补充上代码类型
        py_code_to_test = "```" + code_type + "\n" + code_to_test + "\n```"
        tester_prompt = tester_agent.generate_prompt(code_type, py_code_to_test,test_framework)
        res_json = get_llm_response_local(tester_prompt,model,tokenizer)
        if not res_json :
            continue
